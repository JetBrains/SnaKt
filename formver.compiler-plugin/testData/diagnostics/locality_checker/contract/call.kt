...
fun `assign global function to local function declaration`() {
    val f: (@Borrowed Any) -> Unit = <!LOCALITY_CONTRACT_MISMATCH!>produceGlobalFunction()<!>
}

fun `assign global function to local function variable`() {
    var f: (@Borrowed Any) -> Unit = produceLocalFunction()
    f = <!LOCALITY_CONTRACT_MISMATCH!>produceGlobalFunction()<!>
}
