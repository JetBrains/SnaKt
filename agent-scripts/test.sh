...
run_module() {
    ...
    local rewritten_before failed_before unreadable_before
    rewritten_before="$total_rewritten"
    failed_before="$total_failed"
    unreadable_before="$total_unreadable"
    run_task "$task"
    ...
    if [[ "$MODE" == update ]]; then
        if update_task_failed_unexpectedly "$TASK_STATUS" "$TALLY_STATUS" \
            "$((total_rewritten - rewritten_before))" \
            "$((total_failed - failed_before))" \
            "$((total_unreadable - unreadable_before))"; then
            overall_status=1
