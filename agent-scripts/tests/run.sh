...
assert_eq "update classification: rewritten golden is expected" \
    "" 1 \
    -- bash -c 'source "$1"; update_task_failed_unexpectedly 1 0 0' _ "$LIB_DIR/lib.sh"

assert_eq "update classification: backend error remains a failure" \
    "" 0 \
    -- bash -c 'source "$1"; update_task_failed_unexpectedly 1 0 1' _ "$LIB_DIR/lib.sh"

assert_eq "update classification: unreadable results remain a failure" \
    "" 0 \
    -- bash -c 'source "$1"; update_task_failed_unexpectedly 1 2 0' _ "$LIB_DIR/lib.sh"

assert_eq "update classification: successful task remains successful" \
    "" 1 \
    -- bash -c 'source "$1"; update_task_failed_unexpectedly 0 0 0' _ "$LIB_DIR/lib.sh"
